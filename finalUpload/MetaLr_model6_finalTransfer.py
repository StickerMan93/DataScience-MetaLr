...
#%% tansfer the weight by tuning
ep_ANN.layers[0].trainable = False
ep_ANN.layers[1].trainable = False
ep_ANN.layers[2].trainable = False

#%% check the performance of the transferred model
os.chdir('./ANN_model')

"1. data loading: deficient dataset"
# data pre-process
...
#%% Fine tuning
checkpoint = ModelCheckpoint('./Model/saPred_Transferred_weight.h5', monitor = 'loss', verbose = 1, save_best_only = True)
#es = EarlyStopping(monitor='loss', mode='min', patience = 30)
ep_ANN.fit(x01tr_sa, y01tr_sa, epochs = 20, callbacks = [checkpoint])
ep_ANN.load_weights('./Model/saPred_Transferred_weight.h5')
#%%
...
fig.savefig('./Figure/transferredModel_test.png', dpi = 350, bbox_inches = 'tight')
...
fig.savefig('./Figure/transferredModel_input_operation.png', dpi = 350, bbox_inches = 'tight')